...
- production Compose публикует только frontend на `127.0.0.1:8081`, backend остаётся только во внутренней Docker network;
- production containers используют read-only filesystem, tmpfs runtime paths, `no-new-privileges`, dropped capabilities, PID limits и bounded logs;
- внешний system Nginx завершает TLS, добавляет security headers и применяет отдельный per-IP rate limit к API и PDF;
- production CORS ограничен exact origin `https://quoteflow.stalarvision.ru`.
...
## Перед live deployment обязательно

Репозиторий был публичным с момента создания. MIT License согласована и добавлена. Перед deployment на Beget VPS всё ещё обязательны:

- read-only review ресурсов VPS, свободного loopback-порта, текущих containers и system Nginx;
- dependency и base-image audit без automatic force fix;
- полный Git-history и working-tree scan на secrets и реальные данные;
- production Compose build/smoke и проверка отсутствия host-порта backend;
- exact CORS review для `https://quoteflow.stalarvision.ru`;
- проверка system Nginx TLS, headers, per-IP API/PDF rate limits и safe JSON `429`;
- review frontend rendering, XSS boundaries, PDF library и licenses включённых assets;
- проверка container users, mounts, filesystem, logs и image history;
- backup текущей Nginx configuration и фиксация rollback SHA/image digests;
- отдельное разрешение владельца на фактический deployment.

Подробный план и rollback: `docs/10_DEPLOYMENT_BEGET.md`.

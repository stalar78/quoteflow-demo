...
- GitHub Actions CI для push/PR в `main`: frontend, backend и Docker jobs с read-only permissions, pinned official actions и finite timeouts;
- подготовленный production Compose для Beget VPS: frontend только на `127.0.0.1:8081`, backend без host-порта;
- отдельные bootstrap/final конфигурации системного Nginx для `quoteflow.stalarvision.ru`;
- MIT License.

## Ещё не реализовано

- фактический production deployment;
...
Этапы 0–6 и Stage 7A release-readiness завершены. Репозиторий был публичным с момента создания. Проект лицензирован по MIT: Copyright (c) 2026 Stanislav Larin (Stalar Vision).

Stage 7B находится на этапе подготовки и review deployment-конфигурации для существующего Beget VPS. Планируемый адрес — `https://quoteflow.stalarvision.ru/`. Фактический deployment не выполнялся и разрешается только после отдельной проверки ресурсов сервера и pre-deployment audit.
...
- [Deployment на Beget VPS](docs/10_DEPLOYMENT_BEGET.md)
